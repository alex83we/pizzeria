...
<div class="container">
    ...
    <h3>Angaben gem&auml;&szlig; &sect; 5 TMG</h3>
            @foreach (\Illuminate\Support\Facades\DB::table('firmendaten')->get()->toArray() as $firma)
                @if ($firma->id == 1)
                    <p>{{ $firma->inhaber }}<br />
                        {{ $firma->firmenname }}<br />
                        {{ 'Inh.: ' . $firma->inhaber }}<br />
                        {{ $firma->straße }}<br />
                        {{ $firma->plz . ' ' . $firma->ort }}</p>

                    <h3>Kontakt</h3>
                    <p>{{ 'Telefon: ' . $firma->telefon }}<br />
    ...
    <h3>Haftung f&uuml;r Inhalte</h3> 
            ...
            <h3>Quellenabgaben der verwendeten Bilder:</h3>
            <p>Alle verwendeten Bilder stammen vom Portal https://de.freepik.com und https://pixabay.com, die Urheber sind stockking, 8photo, timolina, freepik, jcomp und daria-yakovleva.</p>
